...
import { PROJECTS } from '../constants'; // Берем данные из обновленного файла
import TextReveal from './TextReveal';

const WorkItem: React.FC<{ item: typeof PROJECTS[0], index: number, onClick: () => void }> = ({ item, index, onClick }) => {
  const ref = useRef(null);

  // Следим за прокруткой карточки относительно окна
  ...
  // Картинка сдвигается от -25% до 25% своей высоты. Это много.
  ...
  return (
    ...
    >
      ...
      >
        {/* ДВИЖУЩИЙСЯ СЛОЙ */}
        <motion.div
          style={{ y, scale: 1.3 }} // scale 1.3 = запас 30% для движения
        ...
        <div className="absolute bottom-6 left-6 right-6 z-10">
          ...
          <TextReveal className="text-2xl text-stone-100 font-serif leading-none">
            {item.title}
          </TextReveal>
        </div>

        <div className="absolute top-4 right-4 bg-stone-100 text-black rounded-full p-3 opacity-0 group-hover:opacity-100 transition-all duration-300 scale-50 group-hover:scale-100 shadow-xl z-20">
      ...
      <div className="flex justify-between items-start border-t border-stone-800 pt-4">
        ...
        <span className="text-stone-600 text-xs font-serif border border-stone-800 px-2 py-1 rounded-full">{item.year}</span>
      </div>
    </motion.div>
  );
};
    ...
    <section id="work" className="py-24 px-6 md:px-16 bg-stone-950 relative">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-24 border-b border-stone-800 pb-8">
        <div className="max-w-2xl">
          <TextReveal className="text-4xl md:text-7xl font-bold tracking-tighter font-serif uppercase mb-2 text-stone-200">
            Истории Исцеления
          </TextReveal>
        </div>

        <span className="text-stone-500 mt-4 md:mt-0 font-serif flex items-center gap-2">
        ...
        {activeCase && (
          ...
          >
            ...
            >
              ...
              <span className="text-yellow-600/80 font-mono text-xs uppercase tracking-widest mb-4 block">
                {activeCase.category}
              </span>

              <h3 className="text-2xl md:text-4xl font-serif text-white mb-8 leading-tight">
                {activeCase.title}
              </h3>
              ...
              <div className="space-y-6 text-stone-300 font-light text-base md:text-lg leading-relaxed">
                <div className="bg-stone-950/50 p-6 rounded-lg border-l-2 border-red-900/50">
                  <h4 className="text-stone-100 font-medium mb-2 font-serif text-xl">Проблема:</h4>
                  <p>{activeCase.shortDesc}</p>
                </div>

                <div className="bg-stone-950/50 p-6 rounded-lg border-l-2 border-green-900/50">
                  <h4 className="text-stone-100 font-medium mb-2 font-serif text-xl">Решение:</h4>
              ...
              <button
                ...
                className="mt-10 w-full py-4 bg-stone-100 text-stone-950 font-medium hover:bg-white transition-all transform hover:scale-[1.02] uppercase tracking-widest text-sm rounded"
              >
